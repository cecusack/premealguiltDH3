...
### anx to phys
```{r}
phys_anx = lme(phys_lead ~ 1 + physuncomfort + anx_state,
            random =~ 1|unique_id,
            data=dat,
            correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
...
### add level 2 ###
phys_anx2 = lme(phys_lead ~ 1 + physuncomfort + anx_state+cent_anx,
            random =~ 1|unique_id,
            data=dat,
            correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
...
phys_anx2_noAR1 = lme(phys_lead ~ 1 + physuncomfort + anx_state+cent_anx,
            random =~ 1|unique_id,
            data=dat,
            #correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
summary(phys_anx2_noAR1)
intervals(phys_anx2_noAR1)

### add random slopes ###
# phys_anx3 = lme(phys_lead ~ 1 + physuncomfort + anx_state+cent_anx,
#             random =~ 1 + anx_state|unique_id,
#             data=dat,
#             correlation= corCAR1(form=~beepcont),
#             na.action = na.exclude)
# summary(phys_anx3)
# intervals(phys_anx3) # NPD go back to 2
# anova(anx_phys2, anx_phys3)

## effect size
lme.dscore(phys_anx3, dat, "nlme")


## pseudo R squared
r2mlm(phys_anx3, bargraph = TRUE)

qqnorm(residuals(phys_anx2))
qqnorm(phys_anx2, ~ranef(., level=1))
plot(phys_anx2)
```
...
### guilt to phys
```{r}
phys_guilt = lme(phys_lead ~ 1 + physuncomfort + guilt_state,
            random =~ 1|unique_id,
            data=dat,
            correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
...
### add level 2 ###
phys_guilt2 = lme(phys_lead ~ 1 + physuncomfort + guilt_state+cent_guilt,
            random =~ 1|unique_id,
            data=dat,
            correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
summary(phys_guilt2)
intervals(phys_guilt2)

phys_guilt2_noAR1 = lme(phys_lead ~ 1 + physuncomfort + guilt_state+cent_guilt,
            random =~ 1|unique_id,
            data=dat,
            #correlation= corCAR1(form=~beepcont),
            na.action = na.exclude)
summary(phys_guilt2_noAR1)
intervals(phys_guilt2_noAR1)
anova(phys_guilt2, phys_guilt2_noAR1)

### add random slopes ###
# phys_guilt3 = lme(phys_lead ~ 1 + physuncomfort + guilt_state+cent_guilt,
#             random =~ 1 + guilt_state|unique_id,
#             data=dat,
#             correlation= corCAR1(form=~beepcont),
#             na.action = na.exclude)
# summary(phys_guilt3)
# intervals(phys_guilt3) # NPD go back to 2
# # anova(anx_phys2, anx_phys3)
# 
# ## effect size
# lme.dscore(phys_anx2, dat, "nlme")
# 
# 
# ## pseudo R squared
# r2mlm(phys_anx2, bargraph = TRUE)

qqnorm(phys_guilt2_noAR1)
qqnorm(phys_guilt2_noAR1, ~ranef(., level=1))
plot(phys_guilt2_noAR1)
```
...
summary(phys_anx2_noAR1)
intervals(phys_anx2_noAR1)
lme.dscore(phys_anx2_noAR1, dat, "nlme")
r2mlm(phys_anx2_noAR1)
```
...
summary(phys_guilt2_noAR1)
intervals(phys_guilt2_noAR1)
lme.dscore(phys_guilt2_noAR1, dat, "nlme")
r2mlm(phys_guilt2_noAR1)
```
